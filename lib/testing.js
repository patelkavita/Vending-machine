...
const areObjEqual = function(expected, actual) {
  const actualKeys = Object.keys(actual);
  const expectedKeys = Object.keys(expected);

  if(areArrayEqual(expectedKeys,actualKeys)) {
    let index = 0;
    while( index < actualKeys.length) {
      if(expected[expectedKeys[index]] !== actual[actualKeys[index]]) {
        return false;
      }
      index++;
    }
...
const assertObj = function(expected, actual, message) {

  if(areObjEqual(expected, actual)) passedTest += 1;
  const passMessage = passedMessage(expected, actual, message);
  const failMessage = failedMessage(expected, actual, message);
  totalTest += 1;
  console.log(areObjEqual(expected, actual)? passMessage : failMessage);
}

let a = {'a' : 1, 'b' : 3, 'c' : 2};
let b = {'a' : 1, 'b' : 3, 'c' : 2};
const messageA = "both a and B are equal";
console.log(assertObj(a,b, messageA));
